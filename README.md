...
- 学生可自行注册，用户名查重、密码哈希保存，禁止公开注册教师账号；注册后使用现有登录接口获取 Token。
- 教师发布任务，登录用户查看任务。
- 教师可分别修改任务标题和描述。
- 学生提交文字成果，并且只能查询、修改自己的成果。
- 学生可分页查询自己的待审核成果，并查看提交总数、审核通过数量。
...
前端联调优先阅读：[前端接口接入手册（B + C 全量后端）](docs/frontend-api-guide.md)，包含当前 23 个应用接口、参数约束、请求/响应示例、权限、错误处理与请求封装。
...
- 已提供学生注册；没有公开教师注册、找回密码、文件上传、自动续期、服务端注销接口。
...
- 当前接口接入以 [前端接口接入手册](docs/frontend-api-guide.md) 与实际运行的 OpenAPI 为准；旧交接文档保留为阶段记录，修改接口前先同步调用方。
